...
float global_resistance;      // lab temperachure
...
void loop() {
  ...
  } else if (goal_resistance < global_resistance && goal_resistance < thermistor_resistance) {  //goal temp hoter than enviornment and current plate temp
    hbridge.hotsurface();                                                                       //heat build plate
  } else {
    hbridge.off();  //cut power to Peltier modules
  }

  delay(1000);  //paus in function
}
